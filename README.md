# Door Safety Stay-Time Classification

## 프로젝트 주제

**자체 구축 영상 데이터셋을 활용한 현관 앞 접근 인물의 체류시간 기반 위험도 분류 모델 개발**

---

## 프로젝트 개요

본 프로젝트는 현관 앞 접근 상황을 촬영한 자체 영상 데이터셋을 구축하고, 영상에서 추출한 문 앞 체류시간과 위치 변화 특징을 기반으로 접근 상황을 `normal`, `suspicious`, `dangerous` 3개 클래스로 분류하는 모델을 개발하는 것을 목표로 한다.

기존 CCTV나 보안 장비는 주로 영상 기록 또는 단순 움직임 감지에 의존하는 경우가 많다. 본 프로젝트에서는 팀원들이 직접 촬영한 현관 앞 접근 상황 영상을 활용하여, 사람의 문 앞 체류시간, 위험구역 체류 비율, 위험구역 진입 횟수, 이동 거리, 평균 속도, 방향 전환 횟수 등의 특징을 추출하고 이를 바탕으로 위험도 분류 모델을 학습한다.
...
- 사람의 문 앞 체류시간과 위치 변화 기반 특징 추출
- 추출된 특징값을 CSV 데이터셋으로 저장
- RandomForest 기반 위험도 분류 모델 학습
- 테스트 영상에 대한 위험도 예측 결과 출력
...
| 클래스 | 의미 | 예시 |
|---|---|---|
| `normal` | 정상적인 접근 또는 통과 상황 | 문 앞 체류시간이 짧고 자연스럽게 이동 |
| `suspicious` | 주의가 필요한 접근 상황 | 문 앞에 비교적 오래 머무르거나 반복적으로 접근 |
| `dangerous` | 위험 가능성이 높은 접근 상황 | 문 가까이에 오래 머물거나 침입 시도 가능성이 높은 패턴 |

본 프로젝트는 문 두드림, 비밀번호 입력 등 세부 행동명을 직접 인식하는 방식이 아니라, 문 앞 위험구역에 머문 시간과 위치 변화 특징을 이용해 위험도를 분류한다.
...
5. 문 앞 위험구역 진입 여부 및 체류시간 분석
6. 영상별 체류시간 및 위치 변화 특징 추출
7. `features.csv` 생성
8. `features.csv`와 `labels.csv`를 이용한 분류 모델 학습
9. 테스트 영상에 대한 위험도 클래스 예측
...
| 이름 | 담당 |
|---|---|
| 음아인 | 영상 기반 인물 탐지 및 체류시간 특징 추출 모듈 |
| 문지민 | 자체 영상 데이터셋 구축 및 라벨링 관리 |
| 송시은 | 위험도 분류 모델 학습 및 평가 |

---

## 역할별 세부 내용

### 음아인 - 영상 기반 인물 탐지 및 체류시간 특징 추출
...
- 현관 앞 위험구역 설정
- 위험구역 진입 여부 판단
- 문 앞 체류시간, 이동 거리, 평균 속도, 방향 전환 횟수 등 위치 기반 특징 추출
- 영상별 특징값을 `features.csv`로 저장
- 전체 영상 폴더를 순회하며 특징값을 자동 생성하는 실행 흐름 구현
- 문 앞 체류시간 중심의 위험도 판단 기준 설계
- Streamlit 기반 시연 UI 구현
- `suspicious`, `dangerous` 감지 결과를 주의 알림함에 누적하는 기능 구현
...
각 영상에서 추출할 주요 특징은 다음과 같다. 특히 `zone_stay_time`과 `zone_ratio`는 문 앞 체류 정도를 나타내는 핵심 특징이다.
...
`features.csv`는 영상에서 추출한 체류시간 및 위치 변화 특징값을 저장한다. `label`은 포함하지 않으며, 이후 `labels.csv`와 `video_name` 기준으로 병합한다.
...
전체 영상 데이터셋에서 체류시간 및 위치 변화 특징을 추출하려면 다음 명령어를 실행한다.
...
웹 UI에서는 프로젝트 영상 선택 또는 영상 파일 업로드를 통해 문 앞 체류시간 분석, 위험도 예측 결과, 분석 캡처 이미지, 주의 알림함을 확인할 수 있다.
...
본 프로젝트에서는 영상 기반 체류시간 및 위치 변화 특징 추출과 위험도 분류 모델 학습에 집중한다. 향후에는 다음 기능으로 확장할 수 있다.
